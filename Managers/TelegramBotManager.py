...
class TelegramBotManager:
    ...
    def __init__(self, token, handlers, log_callback=None):
    ...
    def start(self):
        ...
        self._thread.start()
        self.log_callback("🤖 Telegram bot started")
        return True
    ...
    def _request(self, method, payload=None, timeout=30):
        ...
        try:
            ...
            if not data.get("ok"):
                ...
                self.log_callback(f"⚠️ Telegram API returned not-ok ({method}): {description}")
                return None
            return data.get("result")
        except Exception as exc:
            self.log_callback(f"⚠️ Telegram API error ({method}): {exc}")
            return None
    ...
    def _send_photo(self, chat_id, image_path):
        for attempt in range(1, 4):
            try:
                ...
                self.log_callback(f"⚠️ Telegram sendPhoto error (attempt {attempt}/3): {exc}")
                if attempt < 3:
                    time.sleep(1)
        ...
        try:
            ...
            self.log_callback(f"⚠️ Telegram sendDocument fallback error: {exc}")
            return False
    ...
    def _poll_loop(self):
        ...
        while self._running:
            try:
                ...
                self.log_callback(f"⚠️ Telegram loop error: {exc}")
    ...
    def _dispatch_update(self, update):
        ...
        if not self._pending_updates.acquire(blocking=False):
            ...
            if now >= self._drop_warning_at:
                ...
                self.log_callback("⚠️ Telegram queue is overloaded, dropping update to keep bot responsive")
            return
        ...
        def _run():
            try:
                ...
                self.log_callback(f"⚠️ Telegram update handler error: {exc}")
            finally:
        ...
        try:
            ...
            self._pending_updates.release()
            self.log_callback(f"⚠️ Failed to submit Telegram update to worker: {exc}")
