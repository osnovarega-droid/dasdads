...
class LobbyInstance:
    ...
    def _wait_log_phrase(self, member, phrase="JsFriendLobbyLeaderName", timeout=30.0, poll=0.2, start_cursor=0):
        ...
        print(
            f"❌ [{login}] Не дождались актуальной строки '{phrase}' в течение {timeout:.0f}с после действия leader"
        )
        return False
    ...
    def Collect(self):
        ...
        for bot in self.bots:
            ...
            log_path, start_cursor = self._get_log_cursor(bot)
            if not log_path:
                return False
            ...
            bot.MoveMouse(380, 100)
            time.sleep(0.6)
            if not self._wait_log_phrase(bot, timeout=30.0, start_cursor=start_cursor):
                return False
