...
class Account:
    ...
    def get_fresh_auth_code(self):
        """
        Возвращает «свежий» 2FA код, избегая опасных секунд смены токена.
        Не вводим код на 28/29/30 и 58/59/00 секундах, чтобы не словить просрочку.
        """
        blocked_seconds = {58, 59, 0, 28, 29, 30}

        while True:
            sec = datetime.now().second
            if sec in blocked_seconds:
                time.sleep(0.35)
                continue
            ...
            try:
                ...
                for win in app.windows():
                    ...
                    if any(txt.window_text().strip() == "Enter the code from your Steam Mobile App" for txt in statics) \
                        ...
                        win.set_focus()
                        pyperclip.copy(self.get_fresh_auth_code())
                        time.sleep(0.1)
