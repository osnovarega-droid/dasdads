...
class App(customtkinter.CTk):
    ...
    def _build_license_section(self, parent):
        ...
        telegram_block = customtkinter.CTkFrame(frame, fg_color=BG_CARD_ALT, corner_radius=8, border_width=1, border_color=BG_BORDER)
        telegram_block.grid(row=3, column=0, padx=16, pady=(8, 16), sticky="w")
        telegram_block.grid_columnconfigure(0, weight=0)
        telegram_block.grid_columnconfigure(1, weight=0)
        customtkinter.CTkLabel(telegram_block, text="Telegram bot", text_color=TXT_MAIN, font=customtkinter.CTkFont(size=14, weight="bold")).grid(row=0, column=0, columnspan=2, padx=10, pady=(8, 2), sticky="w")
        customtkinter.CTkLabel(telegram_block, text="Управление panel через Telegram", text_color=TXT_SOFT, font=customtkinter.CTkFont(size=11)).grid(row=1, column=0, columnspan=2, padx=10, pady=(2, 6), sticky="w")
        ...
        self.telegram_bot_status_label.grid(row=3, column=0, columnspan=2, padx=10, pady=(0, 8), sticky="w")
    ...
    def _refresh_telegram_bot_block(self):
        ...
        token = (self.settings_manager.get("TelegramBotToken", "") or "").strip()
        if not token:
            self.telegram_bot_status_label.configure(text='create telegram bot')
            self.telegram_bot_remove_button.grid_forget()
            return
        ...
        self.telegram_bot_status_label.configure(text=status_text)
    ...
    def _start_telegram_bot(self, token):
        ...
        try:
            ...
            self.telegram_bot_manager = TelegramBotManager(token=token, handlers=handlers, log_callback=self.log_manager.add_log)
